...
- Every native `<select>` and its `<option>` elements inside `TCCrystalSurfaceTheme` must use a dark color scheme, dark option background, and light option text so Windows Chrome native popups match the crystal theme; keep this scoped to crystal roots so light controls are not overridden.
