...
class ToolReciveSendInventorySelectPOC: Div {
    ...
    @State var titleText: String = "Buscar Producto"
    ...
    @DOM override var body: DOM.Content {
        ...
        Div{
            ...
            Div {
                ...
                Img()
                    .backButton
                    .hidden(self.$activeMode.map{ ($0 == .searchProduct) })
                    .float(.left)
                    .onClick {
                ...
                H2( self.$activeMode.map{ ($0 == .searchProduct) ? "Buscar Productos" : "Atras" } )
                    .color(self.$activeMode.map{ ($0 == .searchProduct) ? .lightBlueText : .darkOrange })
                    .float(.left)
            ...
            .hidden(self.$activeMode.map {$0 != .createProduct})
        ...
        .height(self.$activeMode.map{ ($0 == .searchProduct) ? 50.percent : 85.percent })
        .top(self.$activeMode.map{ ($0 == .searchProduct) ? 25.percent : 7.5.percent })
...
extension ToolReciveSendInventorySelectPOC {
    ...
    enum ActiveMode: String, Codable {
        ...
        var helpText: String{
            switch self {
            case .searchProduct:
                return "Busca el producto a relacionar con el articulo."
            case .selectDep:
