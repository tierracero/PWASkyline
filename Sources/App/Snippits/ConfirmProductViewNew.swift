...
class ConfirmProductViewNew: Div {
    ...
    /// General inventory seperated by store
    var inventory: [CustStoreProductInventorySale] = []
    
    /// Inventory in concession 
            ...
            Div{
                ...
                Div{
                    ...
                    Div{
                        ...
                        .hidden(self.$selectedItems.map{ !$0.isEmpty })
                        // Select by grid
                        Div{
                            ...
                            H2(self.$selectedItems.map{ "Unidades: \($0.count)" })
                            .marginRight(12.px)
                            .float(.right)
        ...
        if storeIdSelectListener == "concession" {
            ...
            inConcession.forEach { sobj in
                ...
                if !self.selectedInventoryIDs.contains(sobj.id) {
                    ...
                    if let id: UUID = sobj.alocatedTo {
                    ...
                    self.inventoryGrid.appendChild(
                        Div{
                            InputCheckbox(self.$selectedItems.map{ $0.contains(sobj.id) })
                            .marginRight(7.px)
                            .marginLeft(7.px)
                            .height(23.px)
                            .width(23.px)
                        ...
                        .backgroundColor(self.$selectedItems.map{ $0.contains(sobj.id) ? .init(r: 22, g: 22, b: 22) : .init(r: 222, g: 222, b: 222) })
                        .color(self.$selectedItems.map{ $0.contains(sobj.id) ? .init(r: 222, g: 222, b: 222) : .init(r: 22, g: 22, b: 22) })
                        .class(.smallButtonBox)
                        .onClick {
                            if self.selectedItems.contains(sobj.id) {
                                if let index = self.selectedItems.firstIndex(of: sobj.id) {
                                    self.selectedItems.remove(at: index)
                                }
                            }
                            else {
                                self.selectedItems.append(sobj.id)
                            }
                        }
