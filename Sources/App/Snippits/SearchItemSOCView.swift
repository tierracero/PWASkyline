...
class SearchItemSOCView: Div {
    ...
    override func buildUI() {
        ...
        if !soc.a.isEmpty {
            if let pDir = customerServiceProfile?.account.pDir {
                avatar.load("https://intratc.co/cdn/\(pDir)/thump_\(soc.a)")
            }
        }
        
        let termParts = searchTerm.explode(" ")
        
        var valueParts = self.soc.n.explode(" ")
        ...
        valueParts = "\(self.soc.u) \(self.soc.b) \(self.soc.m)".purgeSpaces.explode(" ")
