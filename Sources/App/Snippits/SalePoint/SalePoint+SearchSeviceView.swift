//
//  SalePoint+ServiceSearch.swift
//
//
//  Created by Victor Cantu on 3/15/24.
//
...
extension SalePointView {
    ...
    class SearchSeviceView: Div {
        
        override class var name: String { "div" }
        
        var parsableSOCs: State<[SearchChargeResponse]>
        
        private var callback: ((
        ) -> ())
        
        init(
            parsableSOCs: State<[SearchChargeResponse]>,
            callback: @escaping ((
            ) -> ())
        ) {
            self.parsableSOCs = parsableSOCs
        ...
        @DOM override var body: DOM.Content {
            ...
            Div {
                ...
                Div{
                    ForEach(self.parsableSOCs){ item in
                        ServiceItemRow(soc: item) {
        ...
        override func buildUI() {
            ...
            self.currentIds = parsableSOCs.wrappedValue.map{ $0.i }
                     ...
                     resp.forEach { item in
                         ...
                         self.productResultDiv.appendChild(
                            ...
                            SearchItemSOCView(
                                ...
                                self.parsableSOCs.wrappedValue.append(item)
                                
                            }.hidden(self.parsableSOCs.map{ $0.map{ $0.i }.contains(item.i) })
        ...
        func removeItem(id: UUID) {
            ...
            var items: [SearchChargeResponse] = []
            
            parsableSOCs.wrappedValue.forEach { item in
                if item.i == id {
                    return
                }
                items.append(item)
            ...
            parsableSOCs.wrappedValue = items
