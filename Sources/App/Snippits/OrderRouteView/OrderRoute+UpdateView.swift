...
extension OrderRouteView {
    ...
    class UpdateView: Div {
        
        override class var name: String { "div" }
        
        @State var route: CustOrderRoute
        
        @State var routeItems: [CustOrderRouteItem]
        
        @State var orders: [CustOrder]
        
        var userLocations: [CustLocationManager]
        
        private var removeRoute: ((
            _ routeId: UUID
        ) -> ())
        
        init(
            route: CustOrderRoute,
            items: [CustOrderRouteItem],
            orders: [CustOrder],
            userLocations: [CustLocationManager],
            removeRoute: @escaping ((
                _ routeId: UUID
            ) -> ())
        ) {
            self.route = route
        ...
        override func buildUI() {
            ...
            day = route.day.toString
            
            month = route.month.toString
            
            year = route.year.toString
            
            initialTime = getDate(route.startingRangeAt).time
            
            endingTime = getDate(route.endingRangeAt).time
            
            supervisor = route.supervisor
            
            assignUsers = route.assignUsers
        ...
        override func didAddToDOM() {
            ...
            Dispatch.asyncAfter(0.5) {
                ...
                var routeItemsOrder: [ Int : CustOrderRouteItem ] = [:]
                
                var orderRefrence: [ UUID : CustOrder ] = Dictionary(uniqueKeysWithValues: self.orders.map{ ($0.id, $0) })
                
                var routeItemsOrderCount: [ Int ] = []
                
                self.routeItems.forEach { item in
                    
                    routeItemsOrder[item.indexOrder] = item
                    
                    routeItemsOrderCount.append(item.indexOrder)
                    
                }
                
                routeItemsOrderCount.sort()
                
                routeItemsOrderCount.forEach { cc in
                    
                    guard let item = routeItemsOrder[cc] else {
                        return
                    }
                    ...
                    items.append(.init(
                        id: item.id,
                        orderId: item.order,
                        folio: orderRefrence[item.order]?.folio ?? "N/A",
                        name: orderRefrence[item.order]?.name ?? "N/A",
                        street: orderRefrence[item.order]?.street ?? "N/A",
                        colony: orderRefrence[item.order]?.colony ?? "N/A",
                        city: orderRefrence[item.order]?.city ?? "N/A",
                        state: orderRefrence[item.order]?.state ?? "N/A",
                        country: orderRefrence[item.order]?.country ?? "N/A",
                        zip: orderRefrence[item.order]?.zip ?? "N/A",
        ...
        func processMap() {
            ...
            var routUserPayload: [RoutUserPayload] = []
            ...
            getUsers(storeid: nil, onlyActive: true) { users in
                ...
                do {
                    ...
                    var data = try JSONEncoder().encode(self.items)
                    
                    guard let tragetJson = String(data: data, encoding: .utf8) else {
                        showError(.unexpectedResult, "No se pudo iniciar mapa, error al convertir data a hilo [traget].")
                        return
                    }
                    
                    self.userLocations.forEach { uloc in
                        
                        guard let udata = userRefrence[uloc.relation] else {
                            return
                        }
                        
                        let interval = ((getNow() - uloc.createdAt).toDouble / 60.toDouble).rounded().toInt
                        ...
                        if interval < 60 {
                            timeMessage = "\(interval) \((interval < 2) ? "minuto" : "minutos")"
                        }
                        else {
                            timeMessage = "\(getDate(uloc.createdAt).formatedShort) \(getDate(uloc.createdAt).formatedShort)"
                        }
                        
                        routUserPayload.append(.init(
                            username: "@\(udata.username.explode("@").first ?? udata.username) \(timeMessage)",
                    ...
                    data = try JSONEncoder().encode(routUserPayload)
                    
                    guard let userJson = String(data: data, encoding: .utf8) else {
                        showError(.unexpectedResult, "No se pudo iniciar mapa, error al convertir data a hilo [user].")
                        return
                    }
        ...
        func saveRoute(){
            ...
            API.custRouteV1.update(
                ...
                showSuccess(.operacionExitosa, "Datos Guradads")
...
extension OrderRouteView.UpdateView {
    ...
    struct RoutUserPayload: Codable {
