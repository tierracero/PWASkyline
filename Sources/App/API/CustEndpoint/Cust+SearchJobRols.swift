...
func searchJobRols(
    /// Term to Search
    term: String,
    callback: @escaping ((_ term: String, _ resp: [JobRolsQuick])->())
){
    ...
    let url = baseAPIUrl("https://api.tierracero.co/cust/v1/searchJobRols") + "&term=\(_term)"
    ...
    xhr.onLoad {
        ...
        if let data = xhr.responseText?.data(using: .utf8) {
            do {
                let resp = try decodeAPIResponse([JobRolsQuick].self, from: data)
